...
function validateConfirmPassword() {
  ...
  return validateAndShowError(
    ...
    (val) => val === signUpPasswordInput.value,
...
if (signInActualForm) {
  signInActualForm.addEventListener("submit", async (event) => {
    ...
    if (isValid) {
      const username = signInEmailInput.value.trim()
...
if (signUpActualForm) {
  signUpActualForm.addEventListener("submit", async (event) => {
    event.preventDefault()
    hideMessage()
    let isValid = true
    isValid =
      validateAndShowError(signUpFullNameInput, null, (val) => val.trim().length > 0, "Toʻliq ismni kiriting") &&
      isValid
    isValid =
      validateAndShowError(
        signUpEmailInput,
